...
InstrumentInstanceFormSet = modelformset_factory(
    InstrumentInstance,
    fields=("instrument", 'samples_per_day',
            'identical_copies', 'integrated_or_walkup'),
    extra=1)
...
class EditInstrumentForm(ModelForm):
    class Meta:
        ...
        widgets = {"identical_copies": RangeInput(attrs={"min": 1,
                                                         "max": 10,
                                                         "step": 1,
                                                         "id": "myInstrumentSlider"})}
...
class AddProcessForm(ModelForm):
    class Meta:
        ...
        widgets = {"sample_count": RangeInput(attrs={"min": 1,
                                                     "max": 100,
                                                     "step": 1,
                                                     "id": "mySampleCountSlider"})}
...
class LabAssumptionsDropdownForm(ModelForm):
    class Meta:
        model = Lab
        fields = ('offset', 'current_year', 'max_utilization',
                  'integrated_hours', 'walkup_hours', 'days_per_month' )
        widgets = {"offset": RangeInput(attrs={"min": 1, "max": 5, "id": "mySlider"})}
        help_texts = {
            'offset': None,
        }
...
class FailureRateDateRangeForm(forms.Form):
    date = forms.DateField(input_formats=['%Y-%m-%d', ],
                           ...
                           help_text="""Pick end date for 3 month 
                                        date range. Default is todays date.""")
    def clean_date(self):
        data = self.cleaned_data['date']
        if data > datetime.date.today():
            raise ValidationError(('Invalid date - date in future'))
        return data
...
class OffsetForm(forms.Form):
    Instrument = forms.ChoiceField(choices = tuple(inst_choices), 
                                   help_text="""Pick an instrument type to 
                                                view all data on all of its instances.""")
